...
public class Catalogo implements Serializable {
    ...
    public ArrayList<Articulo> getArticulosByTipo(String tipo) {
        if (tipo.equals("Todos")) {
            return articulos;
        }

        ArrayList<Articulo> articulosByTipo = new ArrayList<>();
        for (Articulo articulo : articulos) {
            if (articulo != null && articulo.getClass().getSimpleName().equalsIgnoreCase(tipo)) {
                articulosByTipo.add(articulo);
            }
        }
        return articulosByTipo;
    }
